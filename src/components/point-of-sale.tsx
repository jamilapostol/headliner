  ...
  return (
    <>
      <div
        ...
        className="mt-3 cursor-pointer rounded-lg bg-accent px-4 py-2.5 text-center text-[12.5px] font-semibold text-canvas hover:bg-accent/85"
      >
        Open point of sale
      </div>
          ...
          <div onClick={(e) => e.stopPropagation()} className="flex max-h-[85vh] w-full max-w-[440px] flex-col rounded-2xl border border-border bg-surface p-6">
            {done ? (
              <div className="py-6 text-center">
                ...
                <div className="mb-5 text-[13px] text-white/55">{money(total)} logged to Finance as merch income.</div>
                <button onClick={close} className="cursor-pointer rounded-[10px] bg-accent px-5 py-2.5 text-[13.5px] font-semibold text-canvas">
                  Done
                </button>
              </div>
            ) : (
              <>
                ...
                <div className="flex-1 overflow-y-auto">
                  {inStock.length === 0 && <div className="py-6 text-center text-[13px] text-white/40">No items in stock.</div>}
                  <div className="flex flex-col gap-2.5">
                    {inStock.map((item) => {
                      const qty = cart[item.id] ?? 0;
                      return (
                        <div key={item.id} className="flex items-center gap-3 rounded-[10px] border border-white/[.08] bg-surface-nested px-3.5 py-2.5">
                          ...
                          <div className="flex flex-none items-center gap-2">
                            <button
                              ...
                              className="grid h-7 w-7 cursor-pointer place-items-center rounded-md border border-white/15 text-[13px] text-white/70 hover:border-white/35"
                            >
                              −
                            </button>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-white/[.08] pt-4">
                  <div className="text-[13px] text-white/55">{unitCount} item{unitCount === 1 ? "" : "s"}</div>
                  <div className="text-[19px] font-bold text-accent">{money(total)}</div>
                </div>
                <button
                  onClick={complete}
                  disabled={unitCount === 0 || pending}
                  className="mt-3 cursor-pointer rounded-[10px] bg-accent py-2.5 text-center text-[13.5px] font-semibold text-canvas disabled:opacity-50"
                >
